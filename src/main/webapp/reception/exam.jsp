    ...
    $("#countdown").timeTo(parseInt(${examPaperTime}), function() {
        ...
        $(".question").each(function () {
            ...
            $.ajax({
                ...
                data:{"subjectId":subjectId,"answer":answer,"studentId":${sessionScope.studentId},"examPaperId":"${examList[0].examPaperInfo.examPaperId}"},
                dataType:"JSON",
                type:"POST",
                success:function () {
...
function find() {
    <c:forEach items="${examList}" var="t" varStatus="status" >
    var subject;
    var index="${status.index+1}";
    <c:if test="${t.subjectInfo.subjectType==0}">
    subject=$("<dd id="+index+" style=\"float: left;display: none; width: 100%;\">\n" +
        "            <p id=${t.subjectInfo.subjectId} subType=${t.subjectInfo.subjectType} class=\"question\">"+index+"${t.subjectInfo.subjectName}"+"</p>\n" +
        "            <p id=\"opA\">\n" +
        "                <span class=\"aswer-p\">\n" +
        "                    <input type=\"checkbox\" id=\"optionA\" class=\"singlePattern\">\n" +
        "                    <span>"+"${t.subjectInfo.optionA}"+"</span>\n" +
        "                </span>\n" +
        "            </p>\n" +
        "            <p id=\"opB\">\n" +
        "                <span class=\"aswer-p\">\n" +
        "                    <input type=\"checkbox\" id=\"optionB\" class=\"singlePattern\">\n" +
        "                    <span>"+"${t.subjectInfo.optionB}"+"</span>\n" +
        "                </span>\n" +
        "            </p>\n" +
        "            <p id=\"opC\">\n" +
        "                <span class=\"aswer-p\">\n" +
        "                    <input type=\"checkbox\" id=\"optionC\" class=\"singlePattern\">\n" +
        "                    <span>"+"${t.subjectInfo.optionC}"+"</span>\n" +
        "                </span>\n" +
        "            </p>\n" +
        "            <p id=\"opD\">\n" +
        "                <span class=\"aswer-p\">\n" +
        "                    <input type=\"checkbox\" id=\"optionD\" class=\"singlePattern\">\n" +
        "                    <span>"+"${t.subjectInfo.optionD}"+"</span>\n" +
        "                </span>\n" +
        "            </p></div>\n"+
        "            </dd>");
    </c:if>

    <c:if test="${t.subjectInfo.subjectType==1}">
    subject=$("<dd id="+index+" style=\"float: left;display: none; width: 100%;\">\n" +
        "            <p id=${t.subjectInfo.subjectId} subType=${t.subjectInfo.subjectType} class=\"question\">"+index+"${t.subjectInfo.subjectName}"+"</p>\n" +
        "            <p id=\"opA\">\n" +
        "                <span class=\"aswer-p\">\n" +
